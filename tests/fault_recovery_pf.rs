//! Integration test: fault_recovery_pf.
//! Verifies Phase 1.1 of PLAN.md: a page fault raised from ring 3 kills
//! the offending process and returns control to the kernel instead of
...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    serial_print!("fault_recovery_pf::page_fault_kills_ring3_process...\t");

    let before_alive = SCHEDULER.processes_alive();
    let pid =
        userland::create_process_from_program(&mut mapper, &mut frame_allocator, userland::CRASH_PF_DEMO)
    ...
    // been reaped. If the page fault handler panicked instead of
    // killing the process, we never get here at all — the panic
    // handler below reports the test as failed.
